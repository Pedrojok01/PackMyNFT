import FallbackIMG from "../../public/img/pack-my-nft-URI2.png";
...
const FALLBACK_IMAGE_URL = FallbackIMG.src;

export const fixIpfsUrl = (nft: EvmNft): string => {
  // Fallback image if no image URL is found
  if (!nft.normalized_metadata?.image) {
    return FALLBACK_IMAGE_URL;
  }

  let url = nft.normalized_metadata.image;

  // If the URL starts with 'ipfs://', convert it to a gateway URL
  if (url.startsWith("ipfs://")) {
    const cid = url.replace("ipfs://", "");
    url = `${IPFS_GATEWAY_URL}${cid}`;
    // Handle cases where the gateway URL already ends with '/ipfs/'
    url = url.replace(/\/ipfs\/ipfs\//, "/ipfs/");
    return url;
  }

  // Handle URLs that contain 'https://' and '/ipfs/'
  ...
  if (hasHttpAndIpfs) {
    ...
    url = url.replace(`https://${ipfsHash}/ipfs/`, IPFS_GATEWAY_URL);
  }
